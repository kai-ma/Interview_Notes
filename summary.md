...
泛型是Java SE 1.5的新特性，泛型的本质是参数化类型，也就是说所操作的数据类型被指定为一个参数。这种参数类型可以用在类、接口和方法的创建中，分别称为**泛型类、泛型接口、泛型方法。** Java语言引入泛型的好处是**安全简单。**
...
[操作系统基础](docs/operating-system/操作系统基础.md)
