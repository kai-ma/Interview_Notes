...
[SpringIOC容器创建过程](https://www.cnblogs.com/dubhlinn/p/10764845.html)
...
1. BeanDefinitionReader读取Resource所指向的配置文件资源，然后解析配置文件。配置文件中每一个<bean>解析成一个BeanDefinition对象，通过BeanDefinitionRegistry，保存到beanDefinitionMap中；
...
public void refresh() throws BeansException, IllegalStateException {
    synchronized (this.startupShutdownMonitor) {
        ...
        //1. if hasBeanFactory, destroyBeans();closeBeanFactory()
        //2. createBeanFactory(); loadBeanDefinitions();
        loadBeanDefinitions
...
### bean的生命周期 在getBean才开始

![img](images/spring-src/beans-life-cicle.jpg)
...
接下来是**最重要的 loadBeanDefinitions(beanFactory)** 方法了，这个方法将根据配置，加载各个 Bean，然后放到 BeanFactory 中。
