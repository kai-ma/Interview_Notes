...
**保证如果两个对象相等，则 hashcode 一定也是相同的。**两个对象相等，对两个对象分别调用 equals 方法都返回 true。当他们作为key添加到`HashMap`中，应该被认为是相同的key，必须有相同的`hashcode`，否则有可能落不到同一个数组位置上，从而添加了两个相同的key。因此，equals 方法被覆盖过，则 `hashCode` 方法也必须被覆盖。

> `hashCode()`的默认行为是对堆上的对象产生独特值。如果没有重写 `hashCode()`，则该 class 的两个对象的hashcode 无论如何都不会相等（即使这两个对象指向相同的数据）
...
1. 一致性：同一个对象每次调用hashCode都应该返回相同的值。
2. 如果两个对象相等equals，则hashcode 一定也是相同的
3. 不相等的对象，不要求hashcode一定相同，但是应该尽可能不同。

object的native hashCode() 的默认行为是对堆上的对象产生独特值，即将对象的内存地址转换为整数之后返回。如果没有重写 hashCode()，则该 class 的两个对象无论如何都不会相等（即使这两个对象指向相同的数据）。

**equals()的相关规定**

1. 自反性：对于任何非空的引用值x，x.equals(x)应该返回true。

2. 对称的：对于任何非空的引用值x和y，当且仅当y.equals(x)返回true时，x.equals(y)也应返回true。

3. 一致性：对于任何非空的引用值x和y，在对象的equals比较中使用的信息没有被修改时，x.equals(y)的多次调用始终返回true或false。

4. 可传递性：x.equals(y)，y.equals(z)，则x.equals(z)。

5. 对于任何非空引用值x，x.equals(null)应返回false。

6. 若不重写equals方法，Object中的equals方法当且仅当两个对象引用同一个对象的时候，返回true。

7. **相等的对象必须有相同的hashcode。因此equals 方法被覆盖过，则 hashCode 方法也必须被覆盖**
...
**将链表转化为红黑树，以减少搜索时间。**当链表长度大于阈值（默认为 8）时，会首先调用 `treeifyBin()`方法。这个方法会根据 HashMap 数组来决定是否转换为红黑树。只有当数组长度大于或者等于 64 的情况下，才会执行转换红黑树操作，以减少搜索时间。否则，就是只是执行 `resize()` 方法对数组扩容。相关源码这里就不贴了，重点关注 `treeifyBin()`方法即可！
...
![image-20210330110508870](images/HashMap/field of hashmap.png)

**TreeNode继承自Node，因此也有next，也可以以链表的方式遍历**，`map.containsValue()`就是利用了这一点来遍历读取value的。同时，`KeyIterator`，`ValueIterator`，`EntryIterator`都继承自`HashIterator`，也是按照table，再按照table中的next遍历的。
...
首先获取到key的hashcode，然后与右移16异或，最后对长度进行取模h & (length-1)。
...
final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
    ...
    if ((tab = table) == null || (n = tab.length) == 0)
        n = (tab = resize()).length;
    if ((p = tab[i = (n - 1) & hash]) == null)  //p = tab[i = (n - 1) & hash]
...
注意：object中的hashCode()是native方法，是对堆上的对象产生独特值，即将对象的内存地址转换为整数之后返回。如果没有重写 hashCode()，则该 class 的两个对象无论如何都不会相等（即使这两个对象指向相同的数据）。

理想的哈希函数应当具有均匀性，即不相等的对象应当均匀分布到所有可能的哈希值上。这就要求了哈希函数要把所有域的值都考虑进来。可以将每个域都当成 R 进制的某一位，然后组成一个 R 进制的整数。
R 一般取 31，因为它是一个奇素数，如果是偶数的话，当出现乘法溢出，信息就会丢失，因为与 2 相乘相当于向左移一位，最左边的位丢失。并且**一个数与 31 相乘可以转换成移位和减法：31*x == (x<<5)-x**，编译器会自动进行这个优化。
...
2. 如果定位到的数组位置有元素就和要插入的 key 比较，如果 key 相同就直接覆盖，如果 key 不相同，就判断 p 是否是一个树节点，如果是就调用`e = ((TreeNode<K,V>)p).putTreeVal(this, tab, hash, key, value)`将元素添加进入。如果不是就遍历链表比较插入(插入的是链表尾部)。

![HashMap.put方法](images/HashMap/HashMap-put方法.png)
...
**推荐阅读：[HashMap 的 7 种遍历方式与性能分析！](https://mp.weixin.qq.com/s/Zz6mofCtmYpABDL1ap04ow)**
...
**TreeNode继承自Node，因此也有next，也可以以链表的方式遍历**。`KeyIterator`，`ValueIterator`，`EntryIterator`都继承自`HashIterator`，也是按照table，再按照table中的next遍历的。**本质上都用的同一个Iterator，同一种遍历顺序。**

同时，`map.containsValue()`就是利用了这一点来遍历读取value的。
